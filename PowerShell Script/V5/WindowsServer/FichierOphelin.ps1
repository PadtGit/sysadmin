...
function Invoke-InstallerOrphanMove {
    ...
    foreach ($InstallerFile in $InstallerFiles) {
        ...
        if (Test-Path -LiteralPath $DestinationPath) {
            $DestinationPath = Join-Path -Path $BackupFolder -ChildPath ('{0}_{1}{2}' -f [System.IO.Path]::GetFileNameWithoutExtension($InstallerFile.Name), (Get-Date -Format 'yyyyMMddHHmmssfff'), [System.IO.Path]::GetExtension($InstallerFile.Name))
        }
