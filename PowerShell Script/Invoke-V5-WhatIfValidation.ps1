...
$BasePath = Join-Path -Path $PSScriptRoot -ChildPath 'V5'
$RepoRoot = Split-Path -Path $PSScriptRoot -Parent
$PowerShellPath = Join-Path -Path $env:SystemRoot -ChildPath 'System32\WindowsPowerShell\v1.0\powershell.exe'

if ([string]::IsNullOrWhiteSpace($ResultPath)) {
    $ResultPath = Join-Path -Path $RepoRoot -ChildPath 'artifacts\validation\v5-whatif-validation.txt'
}
...
Invoke-V5WhatIfValidation -PowerShellPath $PowerShellPath -ScriptPaths $ScriptPaths -ResultPath $ResultPath
