...
Describe 'V5 advanced cleanup' {
    ...
    BeforeAll {
        $script:ModuleInfo = Import-ScriptModuleForTest -RelativeScriptPath 'PowerShell Script\V5\windows-maintenance\Nettoyage.Avance.Windows.Sauf.logserreur.ps1'
    }
    ...
    It 'supports safe preview without elevation and returns the cleanup status contract' {
        ...
        $scriptPath = Join-Path (Get-SysadminMainRepoRoot) 'PowerShell Script\V5\windows-maintenance\Nettoyage.Avance.Windows.Sauf.logserreur.ps1'
