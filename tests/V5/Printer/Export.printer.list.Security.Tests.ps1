Describe 'V5 printer export hardening' {
    ...
    BeforeAll {
        $script:BasicModuleInfo = Import-ScriptModuleForTest -RelativeScriptPath 'PowerShell Script\V5\Printer\Export.printer.list.BASIC.ps1'
    }
    ...
    It 'uses a secured per-user path and unique file name for the basic export preview' {
        $Result = Invoke-WhatIfScriptObject -Shell powershell -RelativeScriptPath 'PowerShell Script\V5\Printer\Export.printer.list.BASIC.ps1'
    ...
    It 'uses a secured per-user path and unique file name for the full export preview' {
        $Result = Invoke-WhatIfScriptObject -Shell powershell -RelativeScriptPath 'PowerShell Script\V5\Printer\Export.printer.list.FULL.ps1'
    ...
    It 'restricts the export directory in code' {
        $BasicContent = Get-Content -LiteralPath (Join-Path (Get-SysadminMainRepoRoot) 'PowerShell Script\V5\Printer\Export.printer.list.BASIC.ps1') -Raw
        $FullContent = Get-Content -LiteralPath (Join-Path (Get-SysadminMainRepoRoot) 'PowerShell Script\V5\Printer\Export.printer.list.FULL.ps1') -Raw
