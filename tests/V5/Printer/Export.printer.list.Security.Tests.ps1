Describe 'V5 printer export hardening' {
    ...
    It 'restricts the export directory in code' {
        $BasicContent = Get-Content -LiteralPath (Join-Path $global:SysadminMainRepoRoot 'PowerShell Script\V5\Printer\Export.printer.list.BASIC.ps1') -Raw
        $FullContent = Get-Content -LiteralPath (Join-Path $global:SysadminMainRepoRoot 'PowerShell Script\V5\Printer\Export.printer.list.FULL.ps1') -Raw
