Describe 'V7 complete cleanup hardening' {
    . (Resolve-Path (Join-Path $PSScriptRoot '..\..\TestHelpers.ps1')).Path

    It 'uses trusted cleanup specs and reparse-point guards' {
        $ScriptPath = Join-Path $script:RepoRoot 'PowerShell Script\V7\windows-maintenance\Nettoyage.Complet.Caches.Windows.ps1'
